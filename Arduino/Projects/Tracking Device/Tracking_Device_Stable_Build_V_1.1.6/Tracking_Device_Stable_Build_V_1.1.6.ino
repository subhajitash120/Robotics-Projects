    ...
    if (gps.encode(Serial.read())) {
      ...
      if (gps.location.isValid()) {
        ...
        if (gps.satellites.value() > 3 && !smsSent) {
          ...
          if (accuracy > 0.0) {
            // Send the GPS information via SMS
...
void loop() {
  ...
  // Process GPS data received from the serial port every second
  ...
  // Print information to serial monitor every 5 seconds
...
// Function to configure GSM module settings
void gsmConfig() {
...
// Function to send GPS information via SMS
